...
```{r ridge-regression2, echo=TRUE}
glmnet.cv <- cv.glmnet(X, Y, nfolds=10, lambda =  lambda.values, standardize=FALSE, intercept=FALSE, alpha=0)
plot(glmnet.cv)
...
min.df <- data.frame("kFold" = df.v[which.min(kFold)],"GLMnet" =df.v[which(lambda.values == glmnet.cv$lambda.min)])
...
plot(degrees_of_freedom, cvm, col=2, pch=19,cex=.80, ylab="MSE", xlab="Degrees of freedom", main="Comparison")

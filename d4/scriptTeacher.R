...
if (1==1){
  ...
  mean.all <- apply(prostate[,],2,mean)
  sd.all <- apply(prostate[,],2,sd)
  
  m.Y.all <- mean.all[9]
  m.X.all <- mean.all[1:8]
  s.Y.all <- sd.all[9]
  s.X.all <- sd.all[1:8]
  
  Y <- prostate$lpsa[train.sample]-m.Y.all
  X <- (as.matrix(prostate[train.sample,1:8])- matrix(m.X.all,nrow=n.train,ncol=p,byrow = TRUE))%*%diag(1/s.X.all)
  
  Y.val <- prostate$lpsa[val.sample]-m.Y.all
  X.val <- (as.matrix(prostate[val.sample,1:8])- matrix(m.X.all,nrow=n.val,ncol=p,byrow = TRUE))%*%diag(1/s.X.all)
}

# Prostate data: Lasso estimation for a given lambda
# 
# lambda <- 0 # OLS estimation
# lambda <- max(apply(X,2,function(x,y){mean(x*y)},y=Y))
lambda <- .5*max(apply(X,2,function(x,y){mean(x*y)},y=Y))
max.iter <- 50
eps.beta <- 1e-5
eps.r <- 1e-5
...
# 
# Lambda.v vector
lambda.max <- max(apply(X,2,function(x,y){mean(x*y)},y=Y))
n.lambdas <- 100
...
op <- par(mfrow=c(2,1))
# MSPE.val 
plot(lambda.v,MSPE.val,ylim=c(min(MSPE.val-sd.SE.val),
                              max(MSPE.val+sd.SE.val)))
...
op <- par(mfrow=c(2,1))
# MSPE.val aginst the log(lambda)
lg.lambda.v <- log(lambda.v[-n.lambdas])
plot(lg.lambda.v,MSPE.val[-n.lambdas],ylim=c(min(MSPE.val-sd.SE.val),
                                             max(MSPE.val+sd.SE.val)))
...
plot(s,MSPE.val,ylim=c(min(MSPE.val-sd.SE.val),
                       max(MSPE.val+sd.SE.val)))

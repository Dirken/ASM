...
```{r, echo=TRUE, fig.height=4, fig.width=12, message=FALSE, warning=FALSE}
finalModel2 <- lm(gross ~ duration + actor3fl 
                  + genre + yearcat + budget:genre 
                  + budget:yearcat, dataset)
...
anova(nullModel, finalModel2)
```
In this case we can see that our model, **passes** the test since our p-value is lower than 0.05. 

## Interpret the final model

Even our final model has an adjusted r-squared about 0.54 (that maybe seems so low), we can assume that does not exist multicollinearity in its variables, one of our purposes in this project.

We rather a simple model with two predictor variables than a complex model with many of them. It seems difficult to predict gross variable in this dataset due its size (just 940 observations) and its outliers. The log transformation that we have applied to numerical variables have a big impact in the performance of our model, it was a great choice. 

We can say that other kind of models should lead to better results than a simple linear regression model.

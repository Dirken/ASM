---
title: "GAM \ffits for hirsutism data"
...
```{r, results="hide"}
hirsutism <- read.table("hirsutism.dat",
                        header=TRUE,
                        sep="\t",
                        fill=TRUE)
```
...
In this assignment, we are going to use the Hirsutism dataset. Hirsutism is the excessive hairiness on women in those parts of the body where terminal hair does not normally occur or is minimal -for example, a beard or chest hair. The data set hirsutism.dat contains artificial values of measures corresponding to some patients in this study. The variables are the following:

Variable name   | Description                          | Values
:------------------:|:-----------------------------------------------------------------------------------------:|:--------------------------:
Treatment| with values 0, 1, 2 or 3. | Factor with 4 levels |
FGm0 | it indicates the baseline hirsutism level at the randomization moment |	Integer |

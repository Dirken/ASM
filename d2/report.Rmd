...
First of all, we are going to write an R function implementing the ridge regression penalization parameter λ choice based on the minimization of the mean squared prediction
error in a validation set (MSPEval(λ)).

```{r lambda_function}

# Input: Matrix x and vector y corresponding to the training sample; matrix xval and vector yval corresponding to the validation set; a vector lambda.v of candidate values for λ.
# Output: For each element λ in lambda.v, the value of MSPEval(λ).
# Additionally you can plot these values against log(1 + λ) − 1, or against df(λ).
...
Now, we will write an R function implementing the ridge regression penalization parameter λ choice based on k-fold cross-validation (MSPEk−CV(λ))

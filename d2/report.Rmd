...
ridge_lambda_search <- function(x, y, x.val, y.val, lambda.v) {
  result <- data.frame(lambda=lambda.v, mspe=rep(0,length(lambda.v)), df=rep(0,length(lambda.v)))
  x.svd <- svd(x)
...
result.valid <- ridge_lambda_search(x, y, x.val, y.val, lambda.v)
# 5-fold and 10-fold CV
# With CV 
x <- scale(data[,1:8], center = T, scale = T)
y <- scale(data[,9], center = T, scale = F)
result.5.cv <- ridge_lambda_search_cv(x, y, cv = 10, lambda.v)
result.10.cv <- ridge_lambda_search_cv(x, y, cv = 10, lambda.v)
# LOOCV and GCV estimates
result.loocv <- ridge_lambda_search_cv(x, y, cv = nrow(x), lambda.v)
result.loocv.gcv <- ridge_lambda_search_loocv_gcv(x, y, lambda.v)
```
